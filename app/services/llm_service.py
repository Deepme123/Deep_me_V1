...
def _to_responses_input(
    ...
    NOTE: content[].type 은 반드시 'input_text' 여야 함.
    """
    blocks = [
        {
            "role": "system",
...
def stream_noa_response(
    *,
    ...
    - 폴백: Chat Completions (백업 모델)
    """
    mdl = (model or MODEL).strip()
    client = OpenAI(timeout=TIMEOUT)

    # 1) Responses API 경로 (권장: gpt-5-mini)
    if _is_reasoning(mdl):
        try:
            ...
            # gpt-5-mini는 temperature 비지원 → 전달하지 않음
            params = dict(model=mdl, input=blocks, stream=True)
            if max_tokens:
                # Responses API는 max_output_tokens 사용
                params["max_output_tokens"] = int(max_tokens)

            stream = client.responses.create(**params)

            for event in stream:
                # 이벤트 타입: response.output_text.delta 등을 처리
    ...
    for bk in [b.strip() for b in DEFAULT_BACKUPS if b.strip()]:
        try:
            ...
            # 파라미터 호환: 일부 모델은 max_tokens, 일부는 max_completion_tokens
            def _try_chat(**kw):
            ...
            # temperature는 일반 챗 모델에서만 옵션 (gpt-5 계열 제외)
            ...
            try:
                chat_stream = _try_chat(**chat_kwargs)
            except BadRequestError as e:
                # 파라미터 호환 재시도 (max_tokens로)
                if "max_completion_tokens" in str(e):
                    chat_kwargs.pop("max_completion_tokens", None)
                    chat_kwargs["max_tokens"] = int(max_tokens or 0)
                    chat_stream = _try_chat(**chat_kwargs)
                else:
                    raise
